...
namespace Ion
{
   ...
   * erase. The erase must be done after the next getStatus request. */
      ...
      void DFUInterface::eraseMemoryIfNeededWithoutErasingAtAll()
      {
        if (m_erasePage < 0)
        {
          return;
      ...
      void DFUInterface::writeOnMemory()
      {
        ...
        else if (Flash::SectorAtAddress(m_writeAddress) >= 0)
        {
          ...
          if (last_memory_flashed >= 0 && current_memory_flashed != last_memory_flashed)
          {
          ...
          m_last_page_erased = m_erasePage;
          m_erasePage = -1;
