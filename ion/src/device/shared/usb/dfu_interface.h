...
namespace USB {
  ...
  public:
    DFUInterface(Device *device, Endpoint0 *ep0, uint8_t bInterfaceAlternateSetting) : Interface(ep0),
                                                                                       ...
                                                                                       m_writeAddress(0),
                                                                                       m_eraseAddress(0),
                                                                                       m_bInterfaceAlternateSetting(
                                                                                               bInterfaceAlternateSetting),
                                                                                       m_isErasingAndWriting(
                                                                                               false),
                                                                                       m_isFirstInternalPacket(
                                                                                               true),
                                                                                       m_isInternalLocked(
                                                                                               true),
                                                                                       m_isFirstExternalPacket(
                                                                                               true),
                                                                                       m_lastMemoryType(
                                                                                               -1),
                                                                                       m_lastPageErased(-1),
  ...
  protected:
    ...
    bool
    processSetupInRequest(SetupPacket *request, uint8_t *transferBuffer, uint16_t *transferBufferLength,
  ...
  private:
    ...
    enum class DFUDownloadCommand {
      GetCommand = 0x00, SetAddressPointer = 0x21, Erase = 0x41, ReadUnprotect = 0x92
    };
   ...
   * host has to wait 1ms before sending a getStatus Request. */
              ...
              m_bwPollTimeout{uint8_t((pollTimeout >> 16) & 0xFF), uint8_t((pollTimeout >> 8) & 0xFF),
                              uint8_t(pollTimeout & 0xFF)}, m_bState((uint8_t) state), m_iString(0) {
      }
...
* linked with dfu.ld, so we cannot access the values. */
    ...
    bool processUploadRequest(SetupPacket *request, uint8_t *transferBuffer, uint16_t *transferBufferLength,
                              uint16_t transferBufferMaxLength);

    // Address pointer
    void
    setAddressPointerCommand(SetupPacket *request, uint8_t *transferBuffer, uint16_t transferBufferLength);
    ...
    // Status
    bool getStatus(SetupPacket *request, uint8_t *transferBuffer, uint16_t *transferBufferLength,
                   uint16_t transferBufferMaxLength);

    bool clearStatus(SetupPacket *request, uint8_t *transferBuffer, uint16_t *transferBufferLength,
                     uint16_t transferBufferMaxLength);
